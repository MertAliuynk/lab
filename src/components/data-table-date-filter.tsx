...
export function DataTableDateFilter<TData>({ column, title, multiple }: DataTableDateFilterProps<TData>) {
	...
	const label = React.useMemo(() => {
		if (multiple) {
			...
			const dateText = hasSelectedDates ? formatDateRange(selectedDates) : "Tarih aralığı seç";
		...
		const dateText = hasSelectedDate ? formatDate(selectedDates[0]) : "Tarih seç";
			...
			<PopoverTrigger asChild>
				<Button variant="outline" size="sm" className="border-dashed">
					{hasValue ? (
						<button
							type="button"
							aria-label={`${title} filtresini temizle`}
							tabIndex={0}
							onClick={onReset}
