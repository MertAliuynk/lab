...
>(({ className, onChange, ...props }, ref) => {
	return (
		<RPNInput.default
			ref={ref}
			className={cn("grid grid-cols-[60px_1fr]", className)}
		...
		<Popover modal>
			<PopoverTrigger asChild>
				<Button
					type="button"
					variant="outline"
					className="flex gap-1 rounded-e-none rounded-s-lg border-r-0 px-3 focus:z-10 h-auto"
					disabled={disabled}
				>
			...
			<PopoverContent className="w-[300px] p-0">
				<Command>
					<CommandInput placeholder="Ülke arama..." />
					<CommandList>
...
const FlagComponent = ({ country, countryName }: RPNInput.FlagProps) => {
	...
	return (
		<span className="flex items-center justify-center overflow-hidden">{Flag && <Flag title={countryName} />}</span>
	);
};
