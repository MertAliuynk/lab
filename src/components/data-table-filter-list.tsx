				...
				>
					<div className="flex flex-col gap-1">
						<h4 id={labelId} className="font-medium leading-none">
							{filters.length > 0 ? "Filtreler" : "Filtre uygulanmadı"}
						</h4>
						<p id={descriptionId} className={cn("text-muted-foreground text-sm", filters.length > 0 && "sr-only")}>
							{filters.length > 0
								? "Satırlarınızı düzenlemek için filtreleri değiştirin."
								: "Satırlarınızı düzenlemek için filtre ekleyin."}
						</p>
					</div>
					{filters.length > 0 ? (
...
function DataTableFilterItem<TData>({
	...
	return (
		<SortableItem value={filter.filterId} asChild>
			<div id={filterItemId} tabIndex={-1} className="flex items-center gap-2" onKeyDown={onItemKeyDown}>
