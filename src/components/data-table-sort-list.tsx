				...
				>
					<div className="flex flex-col gap-1">
						<h4 id={labelId} className="font-medium leading-none">
							{sorting.length > 0 ? "Sıralama kriteri" : "Sıralama uygulanmadı"}
						</h4>
						<p id={descriptionId} className={cn("text-muted-foreground text-sm", sorting.length > 0 && "sr-only")}>
							{sorting.length > 0
								? "Satırlarınızı düzenlemek için sıralamayı değiştirin."
								: "Satırlarınızı düzenlemek için sıralama ekleyin."}
						</p>
					</div>
					{sorting.length > 0 && (
...
function DataTableSortItem({
	...
	return (
		<SortableItem value={sort.id} asChild>
			<div id={sortItemId} tabIndex={-1} className="flex items-center gap-2" onKeyDown={onItemKeyDown}>
				<Popover open={showFieldSelector} onOpenChange={setShowFieldSelector}>
					...
					>
						<Command>
							<CommandInput placeholder="Alanlarda ara..." />
							<CommandList>
