...
export default function DeleteProsthesisType({ data }: PageProps) {
	const router = useRouter();

	const [isOpen, setIsOpen] = useState<boolean>(false);

	const { mutateAsync: deleteProsthesisType, isPending } = api.admin.prosthesisType.delete.useMutation();

	const handleDelete = () => {
		toast.promise(
			deleteProsthesisType({ id: data.id }).then(() => {
				router.refresh();
				setIsOpen(false);
			}),
			{
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						Bu işlem geri alınamaz. Bu işlem sonrasında protez tipinin tüm verileri silinecektir.
					</DialogDescription>
				</DialogHeader>
