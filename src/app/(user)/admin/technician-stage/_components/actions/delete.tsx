...
export default function DeleteTechnicianStage({ data }: PageProps) {
	const router = useRouter();

	const [isOpen, setIsOpen] = useState<boolean>(false);

	const { mutateAsync: deleteTechnicianStage, isPending } = api.admin.technicianStage.delete.useMutation();

	const handleDelete = () => {
		toast.promise(
			deleteTechnicianStage({ id: data.id }).then(() => {
				router.refresh();
				setIsOpen(false);
			}),
			{
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						Bu işlem geri alınamaz. Bu işlem sonrasında teknisyen aşamasının tüm verileri silinecektir.
					</DialogDescription>
				</DialogHeader>
