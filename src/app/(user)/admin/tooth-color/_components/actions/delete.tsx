...
export default function DeleteToothColor({ data }: PageProps) {
	const router = useRouter();

	const [isOpen, setIsOpen] = useState<boolean>(false);

	const { mutateAsync: deleteToothColor, isPending } = api.admin.toothColor.delete.useMutation();

	const handleDelete = () => {
		toast.promise(
			deleteToothColor({ id: data.id }).then(() => {
				router.refresh();
				setIsOpen(false);
			}),
			{
		...
		<Dialog open={isOpen} onOpenChange={setIsOpen}>
			...
			<DialogContent>
				<DialogHeader>
					...
					<DialogDescription>
						Bu işlem geri alınamaz. Bu işlem sonrasında diş renginin tüm verileri silinecektir.
					</DialogDescription>
				</DialogHeader>
